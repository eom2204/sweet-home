...
function CartGoods({ itemId }) {
  const dispatch = useDispatch();

  // Get the list of cart items from Redux
  const cartItems = useSelector((state) => state.cart.cartItems);

  // State to track whether the heart is "active" or not
  const [isCartIconActive, setIsCartIconActive] = useState(false);
  ...
  useEffect(() => {
    setIsCartIconActive(cartItems.includes(itemId));
  }, [cartItems, itemId]);

  // Toggle the heart state on click
  const toggleCartIcon = async (e) => {
    // останавливаем распространение действия onClick родительского елемента (карты с переходом на страницу товара)
    e.stopPropagation();
    // Dispatch actions to Redux based on the heart's state
    if (isCartIconActive) {
      dispatch(removeFromCartAndSync(itemId)); // Removes from Redux & Backend
    } else {
      dispatch(addToCart(itemId)); // Add to favorites in Redux
      dispatch(syncCartWithBackend(cartItems));
    }
    setIsCartIconActive((prev) => !prev); // Toggle the local heart state
  };

  return (
    <svg
      className={`cart__icon-heart ${isCartIconActive ? "active" : ""}`}
    ...
    >
      <path
        className="cart__icon-heart--color"
